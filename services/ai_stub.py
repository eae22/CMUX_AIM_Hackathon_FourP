...
def tensor_to_b64(tensor: torch.Tensor, fmt: str = "JPEG", quality: int = 95) -> str:
...
def apply_protection(image_b64: str, algorithm: str = "FGSM", epsilon: float = 0.03) -> ProtectionResult:
    ...
    # [JND 텍스처 마스킹] 지역 표준편차로 인간 시각 민감도 역가중
    # 매끄러운 피부(낮은 std) → 노이즈 최소화, 머리/눈썹(높은 std) → 노이즈 유지
    orig_np_224 = (input_tensor.detach().squeeze(0).permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)
    gray_224    = cv2.cvtColor(orig_np_224, cv2.COLOR_RGB2GRAY).astype(np.float32)
    mean_sq     = cv2.GaussianBlur(gray_224 ** 2, (15, 15), 0)
    sq_mean     = cv2.GaussianBlur(gray_224, (15, 15), 0) ** 2
    local_std   = np.sqrt(np.maximum(mean_sq - sq_mean, 0))
    MIN_W       = 0.25   # 매끄러운 영역도 최소 25% 노이즈 유지 (AI 효과 보장)
    jnd_weight  = np.clip(local_std / (local_std.max() + 1e-8), MIN_W, 1.0).astype(np.float32)
    jnd_t       = torch.from_numpy(jnd_weight).unsqueeze(0).unsqueeze(0).to(device)  # (1,1,224,224)
    raw_noise_224 = raw_noise_224 * jnd_t  # 채널 방향 broadcast

    # [가우시안 블러] salt-and-pepper 패턴 제거 → 시각적으로 부드러운 노이즈
    noise_np_224 = raw_noise_224.squeeze(0).permute(1, 2, 0).cpu().numpy()
    noise_np_224 = cv2.GaussianBlur(noise_np_224, (3, 3), 0.5)
    raw_noise_224 = torch.from_numpy(noise_np_224).permute(2, 0, 1).unsqueeze(0).to(device)

    # 224×224 노이즈 → 원본 해상도 (bilinear)
    noise_full_np = cv2.resize(noise_np_224, (orig_w, orig_h), interpolation=cv2.INTER_LINEAR)
    noise_full    = torch.from_numpy(noise_full_np).permute(2, 0, 1).unsqueeze(0)

    orig_tensor_full = transforms.ToTensor()(original_pil).unsqueeze(0)
    perturbed        = torch.clamp(orig_tensor_full + noise_full, 0, 1)

    noise_map = noise_full.abs() * 10

    perturbed_224 = torch.clamp(input_tensor.detach() + raw_noise_224, 0, 1)
    ai_perception = _build_ai_perception(perturbed_224)

    return ProtectionResult(
        protected_image_base64=tensor_to_b64(perturbed, fmt="PNG"),
