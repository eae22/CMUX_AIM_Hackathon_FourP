...
function initPage() {
  try {
    ...
    if (stored) {
      ...
      // FGSM은 ResNet50을 공격 → deepfake_vulnerability(ResNet50 softmax)로 보호 효과 측정
      const origVuln  = resp.simulation_metrics?.deepfake_vulnerability?.original_score  ?? 95;
      const protVuln  = resp.simulation_metrics?.deepfake_vulnerability?.protected_score ?? 10;
      const protScore = Math.round(Math.max(0, Math.min(100, (1 - protVuln / Math.max(origVuln, 1)) * 100)));
      _currentRequestId = resp.request_id || null;

      renderResult({
        level:            protScore >= 70 ? 'safe' : protScore >= 40 ? 'warn' : 'danger',
        protectionScore:  protScore,
        similarityBefore: Math.round(origVuln),
        similarityAfter:  Math.round(protVuln),
        photo: {
        ...
        heatmapProtectedSrc:   resp.protection_result?.ai_perception?.grad_cam_heatmap  || '',
...
// bfcache에서 복원될 때 sessionStorage 재읽기
...
function renderResult(d) {
  ...
  const calloutEl = document.getElementById('callout-reduction');
  if (calloutEl) {
    const reduction = d.similarityBefore - d.similarityAfter;
    calloutEl.style.color = clr;
    calloutEl.textContent = '—';
    const t0 = performance.now();
    const dur = 1400;
    function animateCallout(now) {
      const t = Math.min((now - t0) / dur, 1);
      const ease = 1 - Math.pow(1 - t, 4);
      const val = Math.round(ease * reduction);
      calloutEl.textContent = t < 1 ? `−${val}%p` : `−${reduction}%p`;
      if (t < 1) requestAnimationFrame(animateCallout);
    }
    setTimeout(() => requestAnimationFrame(animateCallout), 200);
  }
  ...
  if (d.photo.originalSrc) {
    setTimeout(() => renderAIVision(d.photo.originalSrc, d.photo.protectedSrc, d.heatmapSrc, d.heatmapProtectedSrc || ''), 150);
  }
...
async function fetchProtection(imageB64) {
  setLoading(true);
  try {
    const res = await fetch(`${API_BASE_URL}/api/protect`, {
      method:  'POST',
...
async function renderAIVision(origSrc, protSrc, heatmapSrc, heatmapProtectedSrc) {
  if (!origSrc) return;
  ...
  /* 3. Grad-CAM after (real backend heatmap if available, else simulated) */
  const cafter = document.getElementById('canvas-gradcam-after');
  if (cafter) {
    cafter.width = W; cafter.height = H;
    const ctx = cafter.getContext('2d');

    if (heatmapProtectedSrc) {
      const hmProtImg = await _loadImg(heatmapProtectedSrc);
      if (hmProtImg) ctx.drawImage(hmProtImg, 0, 0, W, H);
    } else {
